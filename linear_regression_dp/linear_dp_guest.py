...
class LinearRDpGuest(ModelBase):
    def __init__(self):
        super(LinearRDpGuest, self).__init__()
        self.batch_generator = Guest()
        self.model_param = LinearDpParam() #初始化参数，将当前算法组件相关的参数载入进来
        self.transfer_variable = HeteroLinRDpTransferVariable() #传输变量
    ...
    def fit(self, data_instances,validate_data = None):
        ...
        #模型初始化
        self.model =model_weight.LinearDpWeightGuest()
        ...
        while iteration <= self.e:
            for data_inst in self.batch_generator.generator_batch_data():
                ...
                fore_gradient = self.model.intermediate_result(data_inst,self.aggregated_forwards,self.model.w)
