...
class PoissonDpWeightGuest():
    ...
    def intermediate_result(self,data_instances,aggregated_forwards):
        """
        ...
        """
        #ir_a = aggregated_forwards.join(data_instances, lambda mu, d: mu - d.label)
        ir_a = aggregated_forwards.join(data_instances, lambda mu, d: d.label - mu)
        return ir_a
...
class PoissonDpWeightHost():
    ...
    def compute_forwards(self,data_instances,w):
        ...
        计算wx内积
        """
        ir_b = data_instances.mapValues(lambda x: np.exp(np.dot(x.features, w)))
        return ir_b
    ...
    def gaussian(self,delta,epsilon,L,e,T,eta,b,k,shape_guest,shape_host):
        """
        生成高斯噪声所需要的loc、sigma
        parameters
        --------------------
        ...
        partial_2_4 = 1 - (shape_guest+shape_host)*k + 1 / 3 * np.square(shape_host+shape_guest)*np.square(k)
        partial_2 = np.sqrt(partial_2_1 + partial_2_2 + partial_2_3) * np.sqrt(shape_guest) * partial_2_4 / b #最大敏感度
        partial_3 = epsilon
        sigma = partial_1 * partial_2 / partial_3
