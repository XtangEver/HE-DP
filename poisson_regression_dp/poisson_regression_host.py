...
from federatedml.linear_model.poisson_regression.hetero_poisson_dp_regression.batch_data import Host
...
class PoissonDpHost(ModelBase):
    def __init__(self):
        super().__init__()
        self.batch_generator = Host()
        self.model_param = PoissonDpParam()
        self.transfer_variable = HeteroPoissonDpTransferVariable()
        self.header = None
        self.model = None
        # 以下三个为传输变量
        self.ir_a = None
    ...
    def fit(self, data_instances):
        ...
        #模型初始化
        self.model = model_weight.PoissonDpWeightHost()
    ...
    def predict(self, data_inst):
        """
        纵向泊松回归的预测部分,由于host这里只作为参与方，所以没有输出
        Parameters
        -------------------
        ...
        wx_host = data_instances.mapValues(lambda x: np.exp(np.dot(x.features, self.data_output)))
